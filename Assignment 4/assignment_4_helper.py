...
class ICPVisualizer(object):
    ...
    def __init__(self, pcA, pcB):
        self.pcA = pcA # scene
        self.pcB = pcB # model
        self.pcA_tr = copy.deepcopy(self.pcA)
    ...
    # def view_icp(self, R, t, frame_id):
    def view_icp(self, R, t):
        ...
        self.vis.poll_events()
        self.vis.update_renderer()
        # self.vis.capture_screen_image(os.path.join("screenshots", f"frame_{frame_id}.png"))

    def plot_correspondences(self, correspondences):
        # model_points_selected_tr = self.pcB_tr[correspondences]
        scene_points_selected_tr = self.pcA_tr[correspondences]
        # new_line_set = o3d.geometry.LineSet.create_from_point_cloud_correspondences(self.scene,
        new_line_set = o3d.geometry.LineSet.create_from_point_cloud_correspondences(self.model,
                                                                                # get_o3d_pc(model_points_selected_tr),
                                                                                get_o3d_pc(scene_points_selected_tr),
                                                                                [(i, i) for i in
